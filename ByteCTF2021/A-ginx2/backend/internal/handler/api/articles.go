...
func QueryArticle(c *gin.Context) {
	...
	if username == "admin" {
		...
		db.DB.Select("`articles`.*, GROUP_CONCAT(`tags`.`tags`) as tags").Table("articles").Joins("LEFT JOIN tags ON articles.id = tags.aid AND (articles.author = ? OR articles.is_public = 1)", username).Where("id = ?", id).Group("id").Find(&article)
	}
	c.JSON(200, gin.H{
		"status": 0,
		"data":   article,
