...
import (
	...
	"bytectf2021/aginx/internal/cache"
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if static.CheckStaticPath(r.URL.Path) { // 判断是否满足静态文件路径
			...
			if err == nil { // 无错误 命中缓存
				...
				w.Header().Set("Cache-Key", strings.ReplaceAll(cache.MakeKey(r.URL.Path), "/", "")) // 欺骗一下, 避免被刷了缓存导致整站挂掉
				w.Write(data)
				return
			}
		}
		if strings.HasPrefix(r.URL.Path, "/static/") { // 判断缓存
			data, key, err := cache.GetCache(r.URL.Path)
			if err == nil { // 无错误 命中缓存
				...
				w.Header().Set("Content-Type", contentType)
				w.Header().Set("Cache-Control", "max-age=31536000")
				w.Header().Set("Cache-Key", strings.ReplaceAll(key, "/", ""))
				w.Write(data)
				return
			}
		}
		...
		if strings.HasPrefix(r.URL.Path, "/static/") { // 保存缓存
			if resp.StatusCode == 200 { // 200 才缓存
				cache.SetCache(r.URL.Path, body)
			}
		}
